...
export default function LoveMessageComponent() {
    ...
    const [usermessage, setMessage] = useState<MessagesThemeOne | null>(null);
    const [loading, setLoading] = useState(true);
    const [time, setTime] = useState(getTimeSince(usermessage?.dateInit ? new Date(usermessage.dateInit) : new Date()));

    useEffect(() => {
        const interval = setInterval(() => {
            setTime(getTimeSince(usermessage?.dateInit ? new Date(usermessage.dateInit) : new Date()));
        }, 1000);
    ...
    useEffect(() => {
        async function fetchMessage() {
            try {
                const res = await fetch(`/api/usermessages/${id}?theme=love`);
    ...
    if (loading) return <CircularProgress style={{ margin: 'auto', color: '#A7AAE1' }} />;
        ...
        <Container ref={scrollRootRef}>
            <MessageContainer>
                <Flex gap="middle">
                    <Card
                        {...sharedCardProps}
                        title={`${usermessage?.creatorName?.toLocaleUpperCase() || ''} & ${usermessage?.destinataryName?.toLocaleUpperCase() || ''}`}
                        styles={stylesCardFn}
                        extra={
                            <Button
                                type="link"
                                styles={{ root: { color: '#A7AAE1' } }}
                                onClick={async () => {
                                    try {
                    ...
                    >
                        <ImageContainer>
                            {usermessage?.imagesUrl && usermessage.imagesUrl.length > 0 && (
                                <Image
                                    src={usermessage.imagesUrl[0]}
                                    alt="test"
                                    width={400}
                                    height={300}
                                    style={{ maxWidth: '100%', margin: '20px auto', height: 'auto', display: 'block', borderRadius: 8 }}
                                />
                            )}
                            <section className="spotify-card">
                                {usermessage?.spotifyLink && (
                                    <SpotifyCard link={usermessage.spotifyLink} />
                                )}
                            </section>
                        </ImageContainer>
                        <Meta {...sharedCardMetaProps} styles={stylesCardMeta} title="Uma história de amor" />
                        <DateArea>
                            <Card styles={stylesCardMeta} className="date-card" title="❤️ Uma data especial ❤️" variant="outlined" style={{ width: '100%' }}>
                                <p>
                        ...
                        <CarouselContainer>
                            <Card styles={stylesCardMeta} className="date-card" title="🪐 Lembranças 🪐" variant="outlined" style={{ width: '100%', height: '100%' }}>
                                {!animComplete && (
                                ...
                                <Carousel
                                    ...
                                    dots={false}
                                >
                                    {usermessage?.imagesUrl?.map((img, idx) => (
                                        <div key={idx}>
                                            <div
                                                style={{
                                                    width: "100%",
                                ...
                                <p className={`carousel ${animComplete ? 'carousel-anim' : ''}`}  >Mural de lembranças</p>
                            </Card>
                        </CarouselContainer>
                    </Card>
                </Flex>
